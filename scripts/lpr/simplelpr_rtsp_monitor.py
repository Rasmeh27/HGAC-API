...
def normalize_dominican_identifier(text: str) -> tuple[str, str, int]:
    ...
    if len(raw) == 7:
        ...
        candidates.extend(
            [_correct_by_position(raw, preferred), _correct_by_position(raw, 3 - preferred)]
        )
        for candidate in dict.fromkeys(candidates):
            if PLATE_ONE_LETTER.fullmatch(candidate) or PLATE_TWO_LETTERS.fullmatch(candidate):
...
def _atomic_write(path: Path, payload: dict, retries: int = 30) -> None:
    ...
    try:
        ...
        pass
    raise OSError(f"No se pudo publicar {path} despues de {retries} intentos") from last_error
...
def build_ignition_payload(
    *, identifier: str, raw_text: str, identifier_type: str, confidence: float,
    camera_id: str, camera_name: str, camera_ip: str, output_path: Path,
    ...
    payload.update({
        ...
        "camera_id": camera_id, "camera_name": camera_name, "camera_ip": camera_ip,
        "frame_path": "", "frame_url": "", "crop_path": "", "crop_url": "",
        "clip_path": "", "rejection_reason": "",
        "consensus_votes": 0, "consensus_total": 0, "consensus_ratio": 0.0,
        "event_id": event_id, "engine": "simplelpr_tracker",
        "raw_result": {
            "identifier_type": identifier_type, "identifier": identifier,
            "raw_text": raw_text, "confidence": round(confidence * 100.0, 1),
...
def _safe_rtsp_label(url: str) -> str:
    parsed = urlsplit(url)
    return f"{parsed.scheme}://{parsed.hostname or '?'}:{parsed.port or 554}{parsed.path}"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument("--camera-id", default=os.getenv("SIMPLELPR_CAMERA_ID", "P1-CARRIL-2"))
    parser.add_argument("--camera-name", default=os.getenv("SIMPLELPR_CAMERA_NAME", "P1 - Carril 2"))
    parser.add_argument("--countries", default=os.getenv("SIMPLELPR_COUNTRIES", DEFAULT_COUNTRIES))
    parser.add_argument("--product-key", default=os.getenv("SIMPLELPR_PRODUCT_KEY") or None)
    parser.add_argument("--min-confidence", type=float, default=float(os.getenv("SIMPLELPR_MIN_CONFIDENCE", "0.80")))
    parser.add_argument("--correction-penalty", type=float, default=float(os.getenv("SIMPLELPR_CORRECTION_PENALTY", "0.08")))
    parser.add_argument("--max-width", type=int, default=int(os.getenv("SIMPLELPR_MAX_WIDTH", "1920")))
    parser.add_argument("--max-height", type=int, default=int(os.getenv("SIMPLELPR_MAX_HEIGHT", "1080")))
    parser.add_argument("--max-in-flight", type=int, default=int(os.getenv("SIMPLELPR_MAX_IN_FLIGHT", "6")))
    parser.add_argument("--frame-stride", type=int, default=int(os.getenv("SIMPLELPR_FRAME_STRIDE", "1")))
    parser.add_argument("--tracker-window", type=float, default=float(os.getenv("SIMPLELPR_TRACKER_WINDOW", "1.5")))
    parser.add_argument("--tracker-idle", type=float, default=float(os.getenv("SIMPLELPR_TRACKER_IDLE", "0.75")))
    parser.add_argument("--tracker-min-frames", type=int, default=int(os.getenv("SIMPLELPR_TRACKER_MIN_FRAMES", "2")))
    parser.add_argument(
        "--dedup-seconds",
        type=float,
        default=float(os.getenv("SIMPLELPR_DEDUP_SECONDS", "30")),
    )
    parser.add_argument("--output", default=os.getenv("IGNITION_LPR_LATEST_PATH", "C:/Users/Public/hgac_lpr.json"))
    parser.add_argument(
        "--event-dir",
...
def main(argv: list[str] | None = None) -> int:
    ...
    tracker = engine.createPlateCandidateTracker(
        simplelpr.PlateCandidateTrackerSetupParms(
            ...
            thumbnailWidth=256, thumbnailHeight=128,
        )
    )
    try:
        video = engine.openVideoSource(
            args.url, simplelpr.FrameFormat.FRAME_FORMAT_BGR24,
    ...
    def publish_tracker_result(tracker_result) -> None:
        ...
        for track in tracks:
            reading = _best_reading(
                track.representativeCandidate, args.min_confidence,
            ...
            payload = build_ignition_payload(
                identifier=identifier, raw_text=raw, identifier_type=identifier_type,
                confidence=confidence, camera_id=args.camera_id,
                camera_name=args.camera_name, camera_ip=camera_ip,
    ...
    try:
        while not _shutdown:
            ...
            pool.launchAnalyze(
                streamId=0, requestId=request_id,
                timeoutInMs=simplelpr.TIMEOUT_INFINITE, frame=frame,
            )
            submitted_count += 1
            result_count += collect_available_results()
