...
def test_detector_prioritizes_compact_plate_over_large_thin_edges() -> None:
    frame = np.zeros((500, 900, 3), dtype=np.uint8)
    cv2.line(frame, (40, 80), (850, 160), (255, 255, 255), 3)
    points = np.array([[330, 280], [510, 275], [515, 365], [325, 370]])
    cv2.fillConvexPoly(frame, points, (255, 255, 255))
    cv2.rectangle(frame, (350, 300), (490, 345), (0, 0, 0), 3)

    candidates = OpenCvPlateDetector(max_candidates=3).detect(frame)

    assert candidates
    first = candidates[0]
    assert 300 <= first.x <= 360
    assert 260 <= first.y <= 310
    assert first.width < 250
    assert first.height < 150
...
def test_format_guided_ocr_correction_prefers_fewest_changes() -> None:
    engine = _engine()

    assert engine._correct_ocr_ambiguities("6601421") == "G601421"
    assert engine._correct_ocr_ambiguities("G601421") == "G601421"
...
def test_read_plate_filters_header_and_returns_serial() -> None:
    ...
    assert result.selected_roi == "serial_middle"
