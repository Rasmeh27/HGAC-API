...
Pensado como puente temporal. Hay dos contratos:

* Eventos discretos (`write_lpr_result`, `write_biostar_result`, ...): un archivo
  nuevo ``<event_id>_<tipo>.json`` bajo `IGNITION_JSON_OUTPUT_DIR`.
* LPR "latest" (`write_lpr_latest`): un único archivo fijo (por defecto
  ``C:/Users/Public/hgac_lpr.json``) que el gateway de Ignition lee en bucle. La
  escritura es **atómica** (archivo temporal + ``replace``) para no chocar con
  Ignition leyendo el archivo al mismo tiempo.

Cuando Ignition consuma directamente el API REST, este módulo se podrá retirar.
"""
...
class IgnitionJsonWriter:
    def __init__(
        ...
        self._lpr_latest_path = (
            Path(lpr_latest_path)
            if lpr_latest_path
            else self._output_dir / "hgac_lpr.json"
        )
    ...
    def write_lpr_latest(self, result: BaseModel) -> Path:
        """Publica la última lectura LPR en el contrato consumido por Ignition.

        Acepta cualquier modelo Pydantic con los campos del módulo LPR formal
        (`LprReadResponse`). Mapea al contrato estable de Ignition: cuando la
        lectura no es PLATE_DETECTED, `plate` queda vacío y `plate_matched=False`,
        sin inventar ni autocompletar caracteres.
        """
